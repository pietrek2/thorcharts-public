	...
	const InitializeData = async () => {
		try {
			...
			const series1 = chartData.map((element: ITVL) => {
				return { x: new Date(element.DAY).getTime() / 1000, y: element.TOTAL_VALUE_BONDED }
			})
			const series2 = chartData.map((element: ITVL) => {
				return { x: new Date(element.DAY).getTime() / 1000, y: element.TOTAL_VALUE_BONDED_USD }
			})
			setDataSeries([
				{
					data: series1,
					strokeWidth: 2,
					name: 'total value bonded',
					type: 'line',
					seriesGroup: '[rune]',
					color: 'green',
					unitSymbol: 'ᚱ'
