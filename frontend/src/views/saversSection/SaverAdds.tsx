...
const SaverAdds = () => {
	...
	const InitializeData = async (color: string) => {
		try {
			const chartData = (await getBackendData('thorchain', 'tc_savers_liquidity_adds')) as ISaversLiqChange[]
	...
	if (dataSeries !== undefined) {
		return (
			<ChartPage
				chartTitle="[TC]: Savers Liquidity Deposits"
				chartDesc="Liquidity deposited to the Saver pools."
				chartDescTitle="Metric Description"
				chartData={dataSeries}
				queryId="56149fad-54f2-4eb9-99be-e434ec1dc0ab"
			/>
		)
	} else {
		return <LoadingChart />
